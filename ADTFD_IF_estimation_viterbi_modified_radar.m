function [fidexmult] = ADTFD_IF_estimation_viterbi_modified_radar(Sig, num,dec_fac)
...
Spec12=zeros(length(Sig)/dec_fac,length(Sig)/dec_fac);
for i = 1:num
%Spec=tfr_stft_high(Sig);

                 [Spec,orienttfd]=HTFD_new_radar(Sig,3,8,64,dec_fac);
...
%c = findridges_neww(Spec,orienttfd,delta);
 ...
 IF=dec_fac*(c)/(2*length(Sig));
IF=interp(IF,dec_fac);
                ...
                Sig=s2;%-extr_Sig(iii);
                 [Spec11,orienttfd]=HTFD_new_radar(extr_Sig,3,8,64,dec_fac); 
