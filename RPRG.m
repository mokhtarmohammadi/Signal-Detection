...
if (~isempty(interset)) %
   ...
   if multinter==1
    ...
    for i = 1:numcp
        ...
        locati1 = interv1(1);%
        %交点2
        comp2 = interset{id(i,numcpj),1};%
        interv2 = interset{id(i,numcpj),2};%
        ...
        if ((locati1 - locati2)^2 + (locafr1 - locafr2)^2) <= 4*thrf^2 %compute the distance of two intersection intervals
            ...
            interset(id(i,numcpj),:) = interset(id(i,1),:);% assign the merging results to the two intersection intervals
        end
    end
